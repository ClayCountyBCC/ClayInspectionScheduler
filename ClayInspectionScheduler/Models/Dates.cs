...
namespace InspectionScheduler.Models
{
  public class Dates
  {
    ...
    public static List<string> GetGracePeriodDate( string id )
    {
      
      var dbArgs = new Dapper.DynamicParameters();
      dbArgs.Add( "@PermitNo", id);

      var sql = @"
          select format(clc.SuspendGraceDt, 'MM/dd/yyyy')
          from clContractor clc
          where clc.ContractorCd = 
          ( 
	          select distinct ContractorId
	          from bpBASE_PERMIT
	          where ContractorId in (select ContractorId from bpASSOC_PERMIT where PermitNo = @PermitNo)
	          union all
	          select distinct ContractorId
	          from bpASSOC_PERMIT
	          where  ContractorId in (select ContractorId from bpBASE_PERMIT where PermitNo = @PermitNo)
          )";


      var gpd = Constants.Get_Data<string>( sql, dbArgs );
      return gpd;
